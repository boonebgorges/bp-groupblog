...
function groupblog_edit_settings() {
	global  $groupblog_blog_id, $errors, $filtered_results;
	$bp       = buddypress();
	...
	if ( ! groupblog_edit_base_settings( $settings['groupblog-enable-blog'], $settings['default-administrator'], $settings['default-moderator'], $settings['default-member'], $settings['page_template_layout'], $group_id, $groupblog_blog_id, $settings['groupblog-silent-add'] ) ) {
 ...
 *
 * @param type $groupblog_enable_blog
 * @param type $groupblog_default_admin_role
 * @param type $groupblog_default_mod_role
 * @param type $groupblog_default_member_role
 * @param type $page_template_layout
 * @param type $group_id
 * @param type $groupblog_blog_id
 * @param type $groupblog_silent_add
 * @return boolean
 *
 * @version 2.0, change arguments order
 */
function groupblog_edit_base_settings( $groupblog_enable_blog, $groupblog_default_admin_role, $groupblog_default_mod_role, $groupblog_default_member_role, $page_template_layout, $group_id, $groupblog_blog_id = null, $groupblog_silent_add = null, ) {
...
function bp_groupblog_upgrade_user( $user_id, $group_id, $blog_id = false ) {
	...
	$user_role = bp_groupblog_get_user_role( $user_id, $blog_id, false );
...
/**
 * bp_groupblog_get_user_role( $user_id, $blog_id, $user_login )
 *
 ...
 * @version 2.0
 */
function bp_groupblog_get_user_role( $user_id, $blog_id, $user_login = false ) {
	global $wpdb;
...
function bp_groupblog_create_screen_save() {
	...
	if ( ! groupblog_edit_base_settings( $enable_group_blog, $groupblog_default_admin_role, $groupblog_default_mod_role, $groupblog_default_member_role, $page_template_layout, $groupblog_group_id, $groupblog_blog_id, $silent_add ) ) {
		bp_core_add_message( __( 'There was an error creating your group blog, please try again.', 'groupblog' ), 'error' );
		bp_core_redirect( trailingslashit( $bp->loggedin_user->domain . $bp->groups->slug . '/create/step/' . $bp->action_variables[1] ) );
	}
}
...
function bp_groupblog_show_blog_form( $blogname = '', $blog_title = '', $errors = '' ) {
	global $groupblog_create_screen, $current_site;
	$bp = buddypress();
...
function bp_groupblog_signup_blog( $blogname = '', $blog_title = '', $errors = '' ) {
	...
	<?php } ?>
		...
		<div id="groupblog-member-options">
			...
			<div class="checkbox">
				<label><input type="checkbox" name="groupblog-silent-add" id="groupblog-silent-add" value="1"
				<?php
				if ( bp_groupblog_silent_add( $group_id ) ) {
					?>
					 checked="checked"<?php } ?>/> <?php _e( 'Enable member blog posting', 'groupblog' ); ?></label>
			</div>
					...
					<tr>
						<td class="available-theme top left">
							...
							if ( ! bp_groupblog_is_blog_enabled( $group_id ) ) {
								?>
								 disabled="true"<?php } ?> name="page_template_layout" id="page_template_layout"  value="magazine" type="radio" 
														<?php
														if ( $page_template_layout == 'magazine' ) {
															echo 'checked="checked"';}
														?>
									 /><label style="display:inline;"> <?php _e( 'Magazine', 'groupblog' ); ?></label>
						...
						<td class="available-theme top">
							...
							if ( ! bp_groupblog_is_blog_enabled( $group_id ) ) {
								?>
								 disabled="true"<?php } ?> name="page_template_layout" id="page_template_layout"  value="microblog" type="radio" 
														<?php
														if ( $page_template_layout == 'microblog' ) {
															echo 'checked="checked"';}
														?>
									 /><label style="display:inline;"> <?php _e( 'Microblog', 'groupblog' ); ?></label>
...
function bp_groupblog_validate_blog_signup() {
	global  $wpdb, $current_user, $blogname, $blog_title, $errors;
...
function bp_groupblog_set_group_to_post_activity( $activity, $args = array() ) {
	...
	if ( ! empty( $id ) ) {
		...
		$activity->action = sprintf( __( '%1$s edited the blog post %2$s in the group %3$s:', 'groupblog' ), bp_core_get_userlink( $post->post_author ), '<a href="' . get_permalink( $post->ID ) . '">' . $post->post_title . '</a>', '<a href="' . bp_get_group_permalink( $group ) . '">' . esc_attr( $group->name ) . '</a>' );

		// This is a new blog post!
	} else {
		$activity->action = sprintf( __( '%1$s wrote a new blog post %2$s in the group %3$s:', 'groupblog' ), bp_core_get_userlink( $post->post_author ), '<a href="' . get_permalink( $post->ID ) . '">' . $post->post_title . '</a>', '<a href="' . bp_get_group_permalink( $group ) . '">' . esc_attr( $group->name ) . '</a>' );
	}
 ...
 */
add_filter(
	'bp_activity_post_pre_comment',
	function( $retval, $blog_id ) {
		$group_id = get_groupblog_group_id( $blog_id );
		if ( empty( $group_id ) ) {
			return $retval;
		}
...
function bp_groupblog_posts() {
	...
	<option value="new_groupblog_post"><?php _e( 'Blog Posts', 'groupblog' ); ?></option>
												 <?php
...
function groupblog_screen_blog() {
	...
	if ( bp_is_groups_component() && bp_is_current_action( apply_filters( 'bp_groupblog_subnav_item_slug', 'blog' ) ) ) {
		...
		if ( isset( $checks['redirectblog'] ) && $checks['redirectblog'] == 1 && ! empty( $home_url ) ) {
			...
			} elseif ( 0 != groups_get_current_group() ) {
